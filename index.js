...
function findByTitle(records, id) {
  if (records.length === 0) {
    throw "Inputted array is empty."
  }
  const titleFound = records.find(record => record.title.toUpperCase() === id.toUpperCase())
  if (!titleFound) {
    return null
  }
  return titleFound
...
function getAllRecordsLongerThanNumberOfSeconds(records, lengthInSeconds) {
  if (records.length === 0) {
    ...
    let recordLength = records.filter((record) => {
